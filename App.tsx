...
const PRESET_MUSIC: MusicTrack[] = [
  { id: 'cinematic', label: 'Cinematic', url: 'https://cdn.pixabay.com/download/audio/2022/03/24/audio_b28764e590.mp3', genre: 'Epic' },
  { id: 'cyberpunk', label: 'Cyberpunk', url: 'https://cdn.pixabay.com/download/audio/2023/05/08/audio_6a2185c6f6.mp3', genre: 'Electronic' },
  { id: 'lofi', label: 'Lofi Chill', url: 'https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808737487.mp3', genre: 'Relax' },
  { id: 'ambient', label: 'Ambient', url: 'https://cdn.pixabay.com/download/audio/2021/11/25/audio_91b1227651.mp3', genre: 'Minimal' },
];
...
const App: React.FC = () => {
  ...
  // Handle music playback in sync with video
  useEffect(() => {
  ...
  const renderAppContent = () => {
              ...
              <>
                ...
                {selectedMusicUrl && (
                  <div className="absolute top-4 right-16 z-20">
                    ...
                    <audio ref={audioRef} src={selectedMusicUrl} className="hidden" />
                  </div>
                )}
              </>
            )}
    ...
    return (
      <div className="h-full overflow-y-auto custom-scrollbar p-6 md:p-8 bg-white dark:bg-zinc-950">
        ...
        <form onSubmit={startProcess} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-[10px] font-bold tabular-nums transition-colors ${inputText.length >= 35 ? 'text-amber-500' : 'text-stone-400 dark:text-zinc-500'}`}>
                    {inputText.length} / 40
                  </span>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                   <button 
                    ...
                    className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all flex items-center gap-2 ${selectedMusicUrl === null ? 'bg-stone-900 border-stone-900 text-white dark:bg-white dark:text-stone-900' : 'bg-transparent border-stone-200 dark:border-zinc-800 text-stone-500 hover:bg-stone-50 dark:hover:bg-zinc-800'}`}
                   >
                   ...
                   {PRESET_MUSIC.map(track => (
                     <button 
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all flex items-center justify-between ${selectedMusicUrl === track.url ? 'bg-stone-900 border-stone-900 text-white dark:bg-white dark:text-stone-900' : 'bg-transparent border-stone-200 dark:border-zinc-800 text-stone-500 hover:bg-stone-50 dark:hover:bg-zinc-800'}`}
                     >
                       <div className="flex items-center gap-2">
                         <Headphones size={14} /> {track.label}
                       </div>
                       <span className="opacity-50 text-[10px]">{track.genre}</span>
                     </button>
                   ))}
                   <div className="col-span-2 mt-2">
                     <button 
                      ...
                      className={`w-full px-3 py-2 rounded-xl text-xs font-medium border border-dashed transition-all flex items-center justify-center gap-2 ${selectedMusicUrl === customMusicUrl && customMusicUrl ? 'bg-stone-100 border-stone-400 dark:bg-zinc-800 dark:border-zinc-600' : 'border-stone-200 dark:border-zinc-800 text-stone-400 hover:bg-stone-50 dark:hover:bg-zinc-800'}`}
                     >
                        <Disc size={14} className={selectedMusicUrl === customMusicUrl && customMusicUrl ? 'animate-spin-slow' : ''} /> 
                        {customMusicUrl ? 'Custom Music Loaded' : 'Upload Audio Loop'}
                     </button>
                     <input type="file" ref={musicInputRef} onChange={handleMusicUpload} accept="audio/*" className="sr-only" />
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-stone-50 dark:bg-zinc-900/50 rounded-xl border border-stone-100 dark:border-zinc-800">
                <p className="text-[10px] leading-relaxed text-stone-400 dark:text-zinc-500">
                  By using this feature, you confirm that you have the necessary rights to any content that you upload. Your use is subject to our <a href="https://policies.google.com/terms/generative-ai/use-policy" target="_blank" rel="noopener noreferrer" className="underline hover:text-stone-600 dark:hover:text-stone-300">Prohibited Use Policy</a>.
                </p>
              </div>
            </div>
          </div>
          <div className="pt-6 border-t border-stone-100 dark:border-zinc-800">
            <button type="submit" disabled={!inputText.trim()} className="w-full py-4 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 font-bold rounded-xl hover:bg-stone-800 dark:hover:bg-white transition-all disabled:opacity-50 shadow-xl shadow-stone-900/10 dark:shadow-white/5 active:scale-[0.99] flex items-center justify-center gap-2 text-lg">
              <Play size={20} className="fill-current" /> CREATE MOTION ART
            </button>
          </div>
        </form>
      </div>
